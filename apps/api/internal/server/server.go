...
func New(ctx context.Context, dsn, staffToken string) (http.Handler, *pgxpool.Pool, error) {
	...
	mux.Handle("POST /v1/agents/doc/draft", authed(s.draftDoc))
	mux.Handle("POST /v1/agents/migrations/suggest", authed(s.migrationSuggest))
	mux.Handle("GET /v1/agents/migrations", authed(s.listMigrations))
	mux.Handle("GET /v1/agents/migrations/{id}", authed(s.getMigration))
	mux.Handle("POST /v1/agents/migrations/{id}/approve", authed(s.migrationApprove))
	mux.Handle("DELETE /v1/agents/migrations/{id}", authed(s.deleteMigration))
	mux.Handle("POST /v1/agents/analyst/query", authed(s.analystQuery))
	mux.Handle("POST /v1/agents/config/sheet", authed(s.configSheet))
	mux.Handle("POST /v1/agents/config/{id}/approve", authed(s.configApprove))
	mux.Handle("GET /v1/agents/finance/guardian", authed(s.financeGuardian))
	mux.Handle("PUT /v1/agents/finance/config", authed(s.putFinanceConfig))
	mux.Handle("GET /v1/agents/finance/config", authed(s.getFinanceConfig))
	mux.Handle("POST /v1/agents/assistant/deck", authed(s.assistantDeck))
	mux.Handle("PUT /v1/domains", authed(s.putDomain))
	mux.Handle("GET /v1/domains", authed(s.listDomains))
	mux.Handle("POST /v1/domains/{id}/verify", authed(s.verifyDomain))
	mux.Handle("DELETE /v1/domains/{id}", authed(s.deleteDomain))
	mux.Handle("PUT /v1/theme", authed(s.putTheme))
	mux.Handle("GET /v1/theme", authed(s.getTheme))
	mux.HandleFunc("GET /v1/portal/host", s.portalHostResolve)
	...
	mux.Handle("POST /v1/time/{id}/submit", authed(s.submitTime))
	mux.Handle("POST /v1/time/{id}/approve", authed(s.approveTime))
	mux.Handle("POST /v1/time/{id}/reject", authed(s.rejectTime))
	mux.Handle("GET /v1/time/pending", authed(s.listPendingTime))
	...
	mux.Handle("PUT /v1/scim", authed(s.putSCIMConfig))
	mux.HandleFunc("POST /scim/v2/Users", s.scimCreate)
	mux.HandleFunc("GET /scim/v2/Users", s.scimGetUser)
	mux.HandleFunc("GET /scim/v2/ServiceProviderConfig", s.scimSPConfig)
	mux.HandleFunc("PATCH /scim/v2/Users/{id}", s.scimPatch)
	mux.HandleFunc("DELETE /scim/v2/Users/{id}", s.scimDelete)
	...
	mux.Handle("GET /v1/projects/{id}/margin", authed(s.getProjectMargin))
	mux.Handle("GET /v1/margins", authed(s.listMargins))
	mux.Handle("GET /v1/margins/{id}/why", authed(s.marginWhy))
	...
	mux.Handle("POST /v1/customers/{id}/invoices/generate", authed(s.generateInvoice))
